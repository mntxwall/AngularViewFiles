...
//  selectedValue = null;
  ...
  doTimeCalculating2(headerIndex: Headerindex, row: string[]){
    ...
    if (this.currentCalculate.geohash === this.preCalculate.geohash){
      let findPhoneGeoHash = this.resultPhonesGeoHashDataTime.filter(e => {
        return (e.phone === this.currentCalculate.phone && e.geohash === this.currentCalculate.geohash)
      });

      //findPhoneGeoHash[findPhoneGeoHash.length - 1].
